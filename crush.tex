...
This document describes how to use a program for testing random number
generators using the TestU01
library~\cite{l2005testu01,l2007testu01,l2009testu01}. It is at the
same time the program's source code and its documentation.  It was
written in the literate programming~\cite{knuth1984literate,
  knuth1994cweb, ramsey1994literate} spirit.  A literate program
interleaves program source code and documentation in the same
document.

When chunks of code are displayed on a page of this document, you will
find an integer on the left margin of that page.  This integer
specifies the page on which the chunk of code is written.  
...
is used.  Such number does not appear when we define a source code
file, as is the case of the chunk {\Tt{}\Rm{}{\it{}crush}.{\it{}c}\nwendquote}, the first chunk
defined.  So, when you're reading a chunk, you can look to the right
and you will know which page that chunk is used.
...
The program being implement in this document is called {\Tt{}\Rm{}{\it{}crush}\nwendquote}.
...
sensible to do, we must stop\footnote{Stopping here is a problem if
  you're, say, testing randomness from an RNG whose data is in a file
  on disk.  As long as your file contains sufficient data for the
  chosen battery of tests of TestU01, you should have no problem.
  This program will consome your file as much it needs to and produce
  a complete report.  If, however, your file doesn't contain enough
  data for the chosen battery, an error will be produced and no report
  will be given.  In such situations, the package {\em PractRand} is
  more convenient than the library TestU01 because it incrementally
  tests your data, using more and more data from your file up until
  your generator fails a test, when it stops --- going up to a maximum
  of 32TiB of data.}.
  ...
  {\bf{}if} ({\it{}pos} \begin{math}\geq\end{math} {\it{}limit}) {\nwlbrace}
    ...
    {\bf{}if} ({\it{}limit} \begin{math}\equiv\end{math} 0) {\nwlbrace}
      ...
      {\bf{}if} ({\it{}feof}({\it{}stdin}) \begin{math}\neq\end{math} 0) {\nwlbrace}
        {\it{}printf}({\tt{}"generator produced eof{\char92}n"});
        {\bf{}exit}(0);
      {\nwrbrace}
    {\nwrbrace}
...
\nwixlogsorted{i}{{\nwixident{program{\_}name}}{program:unname}}%
\nwixlogsorted{i}{{\nwixident{usage}}{usage}}%
\nwbegindocs{11}\printbibliography
